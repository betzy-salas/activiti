...
public abstract class ServiceTaskProcessor implements JavaDelegate {
    ...
    public void execute(DelegateExecution execution) {
        ...
        ExternalServicesConfig.ServicesDefinition definition = getConfig().getByServiceName(name);

        if (definition == null) throw new ExternalServicesException("Missing config for service: " + name, new Throwable().getCause());
        ...
        if (mapper != null) {
            try {
                if (response == null || response.getBody() == null || response.getBody().isBlank()) {
                    System.out.printf("⚠️ Activity '%s' received an empty or null response. Mapping variables with null values.%n", activityId);
            ...
            System.out.printf("ℹ️ Activity '%s' has no mapper configured. Stored raw response as 'responseBody'.%n", activityId);
        }
    }
